	...
	for (UINT i = 0; i < MAX_LAYER; ++i)
	{
		...
		for (size_t i = 0; i < vecParent.size(); ++i)
		{
			...
			while (!list.empty())
			{
				...
				if (_Name == pObject->GetName().c_str())
				{
					return pObject;
				}
			}
...
vector<wstring>& CLevel::GetAllObjectNames()
{
	m_vecAllObjectNames.clear();

	for (UINT i = 0; i < MAX_LAYER; i++)
	{
		vector<CGameObject*> layerObjs = m_Layer[i]->GetObjects();
		for (int i = 0; i < layerObjs.size(); i++)
		{
			m_vecAllObjectNames.push_back(layerObjs[i]->GetName());
		}
	}

	return m_vecAllObjectNames;
}
