...
void SE_AtlasView::SelectCheck()
{
	...
	// 마우스 오른쪽 클릭 체크
	if (ImGui::IsMouseClicked(ImGuiMouseButton_Right) && ImGui::IsWindowFocused(ImGuiFocusedFlags_RootAndChildWindows))
	{
		Vec2 vPixelPos = Vec2(ImGui::GetMousePos().x, ImGui::GetMousePos().y);
		ImVec2 vDiff = ImVec2(vPixelPos.x - ImageRectMin.x, vPixelPos.y - ImageRectMin.y);
		vPixelPos = Vec2(vDiff.x / m_Ratio, vDiff.y / m_Ratio);
		m_MouseLT = ImVec2(vPixelPos.x, vPixelPos.y);
	}

	if (ImGui::IsMouseReleased(ImGuiMouseButton_Right) || ImGui::IsMouseDown(ImGuiMouseButton_Right))
	{
		Vec2 vPixelPos = Vec2(ImGui::GetMousePos().x, ImGui::GetMousePos().y);
		ImVec2 vDiff = ImVec2(vPixelPos.x - ImageRectMin.x, vPixelPos.y - ImageRectMin.y);
		vPixelPos = Vec2(vDiff.x / m_Ratio, vDiff.y / m_Ratio);
