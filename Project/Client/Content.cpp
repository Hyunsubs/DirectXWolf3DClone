...
void Content::DropObjectToLevel(DWORD_PTR _Param)
{
	HWND mainWnd = CEngine::GetInst()->GetMainWnd();
	ImGuiIO& io = ImGui::GetIO();

	POINT window_starting = {};
	ClientToScreen(mainWnd, &window_starting);
	// ImGui Mouse Pos - window Rect left top 기본 패딩 고려해서 뺄셈 진행할 것
	// 
	POINT windowSize = CEngine::GetInst()->GetWindowSize();
	Vec2 cursorPos = Vec2(io.MousePos.x, io.MousePos.y);
	
	Vec2 curPos = Vec2(cursorPos.x - window_starting.x, cursorPos.y - window_starting.y);

	bool insideBox = curPos.x >= 0.f && curPos.x <= windowSize.x && curPos.y >= 0.f && curPos.y <= windowSize.y;

	CLevel* pCurLevel = CLevelMgr::GetInst()->GetCurrentLevel();
	TreeNode* pTargetNode = (TreeNode*)_Param;
	Ptr<CAsset> pAsset = (CAsset*)pTargetNode->GetData();
	if (nullptr != pCurLevel && 
		LEVEL_STATE::STOP == pCurLevel->GetState() && 
		pAsset->GetAssetType() == ASSET_TYPE::PREFAB &&
		insideBox)
	{
		Vec3 EditorPos = CEditorMgr::GetInst()->GetEditorCamPos();
		Vec2 TransformedPos = Vec2(EditorPos.x - (windowSize.x / 2), EditorPos.y + (windowSize.y / 2));
		Ptr<CPrefab> pPrefab = (CPrefab*)pAsset.Get();
		CGameObject* CloneObj = pPrefab->Instantiate();
		CloneObj->Transform()->SetRelativePos(TransformedPos.x + curPos.x, TransformedPos.y - curPos.y, 1000.f);
		CreateObject(CloneObj, 0);
	}
}
