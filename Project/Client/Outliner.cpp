...
void Outliner::Update()
{
	...
	if (ImGui::Button("RenewLevel"))
	{
		RenewLevel();
	}

	if (m_deleteObj != nullptr && m_deleteObj->TileMap() != nullptr)
	{
		CLevel* pCurLevel = CLevelMgr::GetInst()->GetCurrentLevel();
		for (int i = 0; i < MAX_LAYER; i++)
		{
			vector<CGameObject*> vecObjs = pCurLevel->GetLayer(i)->GetObjects();
			for (int j = 0; j < vecObjs.size(); j++)
			{
				if (vecObjs[j]->RayCaster2D() != nullptr)
				{
					CGameObject* TargetCaster = vecObjs[j];
					TargetCaster->RayCaster2D()->SetTargetTileMap(nullptr);
				}
			}
		}
		DeleteObject(m_deleteObj);
		m_deleteObj = nullptr;
...
void Outliner::PopupMenu(DWORD_PTR _Param)
{
	...
	CGameObject* pGameObj = (CGameObject*)pTargetNode->GetData();


	if (ImGui::MenuItem("Delete"))
	{
		m_deleteObj = pGameObj;
